...
export function useAgentReview(params: {
  ...
  /** The workspace the review target derives from; every start and poll RPC
   * carries it so the server can bind the reviewed worktree to the parent
   * Agent's workspace. */
  workspaceId: string;
  scope: ReviewScope;
  baseRef: string;
   ...
   * READONLY_REVIEW_POLL_CAP_MS. Every poll repeats the start-time
   * workspace/agent binding alongside the request capability; the server
   * refuses a poll that does not match the recorded binding. Each poll
   * landing re-checks the monotonic analysis run and the active locale so a
   * superseded or locale-cleared request never applies stale state; on cap
   * expiry a clear error is thrown for the caller to surface.
   */
  const pollUntilDone = useCallback(async (
    ...
    while (true) {
      const result = await pollAiReviewRpc({ requestId, workspaceId: boundWorkspaceId, agentId: boundAgentId });
      if (run !== analysisRunRef.current || localeRef.current !== requestedLocale) return;
