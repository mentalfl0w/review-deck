...
import { severityColor, withAlpha, type Finding, type PanelLayout, type PanelTheme, type Severity } from "../tools";
import { computeTooltipPosition, createTooltipVisibilityMachine, snapWindowFrame, windowFrameEquals, type WindowFrame } from "./tooltip";
import { focusMovedIntoOverlay, getWindowViewport, scheduleAnimationFrame, subscribeDocumentKeyDown, subscribeWindowEvent } from "../web";

/** Kept exported for compatibility; the machine itself lives in
 * ./tooltip (pure, unit-testable). */
export { createTooltipVisibilityMachine } from "./tooltip";
...
function TooltipOverlay({ targetRef, text, theme, layout, onClose, testID, open }: {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      if (pending) return;
      pending = true;
      cancelScheduledFrame = scheduleAnimationFrame(() => {
        pending = false;
        if (alive) measure();
      });
    };
    measure();
    // Window scroll/resize drive re-measurement on web only; the adapter
    // unsubscribes are no-ops on native, where the heartbeat below keeps the
    // bubble pinned instead.
    const unsubscribeScroll = subscribeWindowEvent("scroll", schedule, true);
    const unsubscribeResize = subscribeWindowEvent("resize", schedule);
  ...
  useEffect(() => {
    if (native) return;
    const update = () => {
      const size = getWindowViewport();
      if (size !== null) setViewport(size);
    };
    update();
    return subscribeWindowEvent("resize", update);
  }, [native]);

  // Web Escape-to-close (the native Modal path closes via onRequestClose).
  useEffect(() => {
    if (!open || native) return;
    return subscribeDocumentKeyDown((key) => {
 ...
 * NativeSyntheticEvent instead of a weak all-optional type). The
 * relatedTarget value is opaque here — the DOM walk lives in ../web. */
...
function useTooltipVisibility(onClose: () => void) {
  ...
  const focusOut = useCallback(
    (event: TooltipBlurEvent) => {
      ...
      if (focusMovedIntoOverlay(event.relatedTarget, overlayTestID)) return true;
