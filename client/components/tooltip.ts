...
export const defaultTooltipTimers: TooltipTimerHost = {
  ...
  clearTimeout: (handle) => {
    // The handle is opaque by contract: pass it back to the runtime
    // clearTimeout unchanged, whatever concrete type the runtime produced.
    const runtimeHandle = handle as never;
    clearTimeout(runtimeHandle);
  },
};
